...
describe SpotifyClient do
  ...
  it 'gets a user\'s saved playlists' do
    ...
    user_playlists = @client.get_user_playlists("1214851524", max_num)
  ...
  it 'returns the tracks from a playlist' do
    ...
    playlist_tracks = @client.get_playlist_tracks("1214851524", playlist_hash["id"], max_num)
  ...
  it 'gets a playlist\'s id' do
    playlist = File.read('spec/helpers/spotify_playlist_object.json')
    expect(@client.get_playlist_id(playlist)).to eq("1d5gw19rQpEgNRlSWBhMGt")
  end
end